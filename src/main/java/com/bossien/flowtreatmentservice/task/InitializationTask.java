...
public class InitializationTask {
    Logger logger = LoggerFactory.getLogger(InitializationTask.class);
    @Autowired
 ...
 //--------------------------------------------------------------------------------
        ...
        for (Map.Entry<String, Long> stringLongEntry : userCompanyIdsMap.entrySet()) {
            ...
            concurrencyExecutor.run(new Runnable() {
                @Override
                public void run() {
                    operationSumService.saveCompanyAndUserRankBatch(usersId, companyId.toString(), 0.00, 0.00);
                }
            });
        }
    ...
    public List<Long> getProvinceIds() {
        ...
        if (lowCountryIds != null && lowCountryIds.size() > 0) {
            ...
            logger.info("getProvinceIds,nosearch country");
        }
        ...
        for (Company company : byPidIs) {
            List<Long> longs = MemoryClient.instance.getCompanyIdsCache().get("index_" + company.getId());
